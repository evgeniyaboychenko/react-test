...
const Converter = observer(() => {
  ...
  return (
    <section className="сonverter">
      ...
      <Currencies
        CurrencyItem={rootStore.currencyStore.currencyCards}
        onCurrencyClick={rootStore.currencyStore.onCurrencyClick}
      />
      ...
      <p className="сonverter__message">
        {rootStore.converterStore.cardСount === 5
          ? "Количество карточек слишком большое. Удалите ненужные карточки с парами валют. И снова выберете необходимую пару валют"
          : ""}
      </p>
    </section>
  );
